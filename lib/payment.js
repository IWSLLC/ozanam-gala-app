...
var payment = function() {
  ...
  self.redirect_urls = {
    return_url: (process.env.ENABLESSL == 'yes' ? 'https://' + process.env.HOSTNAMESSL : 'http://' + process.env.HOSTNAME) +
    '/register/finish',
    cancel_url: (process.env.ENABLESSL == 'yes' ? 'https://' + process.env.HOSTNAMESSL : 'http://' + process.env.HOSTNAME) +
...
exports.start = function(registration, done) {
  ...
  if (registration.order.level !== 10)
    items.push({
          quantity: 1,
          name:'Ozanam Hollywood Holiday Gala Sponsorship',
          price:number.formatMoney(pricing.getSponsorshipAmount(registration.order.level)),
          currency:'USD'
        })
  ...
  if (registration.order.extraSeats)
    items.push({
          ...
          name:'Extra Seats',
          price:number.formatMoney(pricing.getExtraSeatsPrice()),
          currency:'USD'
        })
  ...
  temp.transactions.push({
    ...
    description: 'Ozanam Hollywood Holiday Gala registration.',
    item_list: {
      items: items
    }
  })
