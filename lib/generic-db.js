...
genericDb.prototype.new = function() { return new this.record()}
...
genericDb.prototype.find = function(query, done) {
  ...
  sharedMongo.open(function(err,db) {
    ...
    collection.find(query).toArray(function(err,docs) {
      ...
      rdocs = _.forEach(docs,self.mergeSync(doc))
      done(null, rdocs)
    })
  })
}
...
genericDb.prototype.findAndModify = function(query, update, sort, options, done) {
  var self = this
  sharedMongo.open(function(err,db) {
    if (err) return done(err)
